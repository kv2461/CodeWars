...
// umbrellas he needs to start with in order that he never gets wet. He can start with some umbrellas at home and some at work, 
// but the output is a single integer, the minimum total number.

// The input is an array/list of consecutive half-day weather forecasts. So, e.g. 
// the first value is the 1st day's morning weather and the second value is the 1st day's afternoon weather. The options are:
...
function minUmbrellas(weather) {
    let noRain = ['clear','sunny','cloudy','overcast','windy'];
    ...
    for (let i = 0;i<weather.length;i++){
      if(rain.includes(weather[i]) && !hasUmbrella && i % 2 === 0) {
        hasUmbrella = true;
        if (umbrellaHome>0){
          umbrellaHome--;
        } else {
      ...
      } else if (rain.includes(weather[i]) && !hasUmbrella && i % 2 === 1) {
        hasUmbrella = true;
        if (umbrellaWork>0){
          umbrellaWork--;
        } else {
      ...
      } else if (noRain.includes(weather[i]) && hasUmbrella) {
        if (i % 2 === 1) {
          umbrellaWork++;
        } else if (i % 2 === 0){
          umbrellaHome++;
