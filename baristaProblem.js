...
// if you could handle all the orders with only one coffee machine in such a way that the sum of all the waiting times of 
// the customers is the smallest possible, he will give you a substantial raise.
...
// Now you have a list coffees of the orders and you write down next to each of the orders the time you 
// need to brew each one of those cups of coffee.
...
// Ignore the time you need to serve the coffee and the time you need to take the orders and 
// write down the time you need to make each one of them.
...
//  so in total 9 minutes. The third customer, by the same logic, is about to wait 9 minutes (for the first two customers) +
//   2 more minutes(cleaning) + 2 minutes (his coffee brewing time). 
...
// Special Thanks to the great Discord community for helping with the creation of this kata and also to the programmers that 
// helped a lot in the " discuss " section.




function barista(coffees){
    let sorted = coffees.sort((a,b)=> a>b?1:-1);
    let totalTime = 0;
