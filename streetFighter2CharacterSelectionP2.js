...
// Empty space
// The first character of the first row (Ryu) is not aligned with the first of the second row (Balrog) but 
// with the second (Ken) and the same goes with the other side; therefore we need to introduce something new, 
...
// with spaces that won't be selectable. In this case we need 2 empty spaces 
// (3 rows x 6 columns = 18 slots, 18 slots - 16 characters = 2 empty spaces). Like this:

// |        | Ryu    | E.Honda  | Blanka  | Guile   |         |
// | Balrog | Ken    | Chun Li  | Zangief | Dhalsim | Sagat   |
// | Vega   | T.Hawk | Fei Long | Deejay  | Cammy   | M.Bison |
// The moves of the selection cursor are the same as before: rotate horizontally but stop vertically. 
// When you find empty spaces (1 or more) you need to skip them if you approach them horizontally and 
// get to the next selectable slot with the next fighter on the left or right; and 
// if you approach them vertically you need to just stop and stay where you are.
...
// Output
// The output is the same as before: the list of characters that have been hovered by the selection cursor after each move, 
// successful or not.
...
function superStreetFighterSelection(fighters, position, moves){
    ...
    let verticalLength = fighters.length-1;
    let horizontal = position[1];
    let horizontalLength = fighters[0].length-1;
    let touchedFighters = [];
    for(let i=0; i<moves.length;i++){
      if (moves[i]==='up'){
        ...
        if(fighters[vertical][horizontal]===""){
            vertical ++;
            touchedFighters.push(fighters[vertical][horizontal])
          } else {
            touchedFighters.push(fighters[vertical][horizontal])
          }
      }
      ...
      if (moves[i]==='left'){
        ...
        while (fighters[vertical][horizontal]===""){
          horizontal--;
          if (horizontal<0){
          horizontal = horizontalLength;
          }
      ...
      if (moves[i]==='right'){
        horizontal++;
        if (horizontal>horizontalLength){
          horizontal = 0;
        }
