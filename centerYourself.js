...
// Task
// Implement a function center that takes a string strng, an integer width, and an optional character fill (default: ' ')
//  and returns a new string of length width where strng is centered and on the right and left padded with fill.
...
function center(strng, width, fill=' ') {
    ...
    let strngArray = [];
    let lengthDiff = width - strng.length;
    if (lengthDiff % 2 === 1) {
      strngArray.push(fill);
      strngArray.push(strng);
      console.log(strngArray)
      for (let i=0;i<(lengthDiff-1)/2;i++) {
        strngArray.push(fill);
        strngArray.unshift(fill);
      }
